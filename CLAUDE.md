...
- **All six line searches are comparably robust**, as is `DogLeg`: across the twelve
  implicit-midpoint sweeps (108 runs each) every one of them converges for the large
  majority of runs, `Bisection` most of all. `Picard` is the only real outlier.
